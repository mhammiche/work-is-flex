# frozen_string_literal: true

require 'rails_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to test the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator. If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails. There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.

RSpec.describe '/booking_requests', type: :request do
  # This should return the minimal set of attributes required to create a valid
  # BookingRequest. As you add validations to BookingRequest, be sure to
  # adjust the attributes here as well.
  let(:valid_attributes) do
    skip('Add a hash of attributes valid for your model')
  end

  let(:invalid_attributes) do
    skip('Add a hash of attributes invalid for your model')
  end

  describe 'GET /index' do
    it 'renders a successful response' do
      BookingRequest.create! valid_attributes
      get booking_requests_url
      expect(response).to be_successful
    end
  end

  describe 'GET /show' do
    it 'renders a successful response' do
      booking_request = BookingRequest.create! valid_attributes
      get booking_request_url(booking_request)
      expect(response).to be_successful
    end
  end

  describe 'GET /new' do
    it 'renders a successful response' do
      get new_booking_request_url
      expect(response).to be_successful
    end
  end

  describe 'GET /edit' do
    it 'renders a successful response' do
      booking_request = BookingRequest.create! valid_attributes
      get edit_booking_request_url(booking_request)
      expect(response).to be_successful
    end
  end

  describe 'POST /create' do
    context 'with valid parameters' do
      it 'creates a new BookingRequest' do
        expect do
          post booking_requests_url, params: { booking_request: valid_attributes }
        end.to change(BookingRequest, :count).by(1)
      end

      it 'redirects to the created booking_request' do
        post booking_requests_url, params: { booking_request: valid_attributes }
        expect(response).to redirect_to(booking_request_url(BookingRequest.last))
      end
    end

    context 'with invalid parameters' do
      it 'does not create a new BookingRequest' do
        expect do
          post booking_requests_url, params: { booking_request: invalid_attributes }
        end.to change(BookingRequest, :count).by(0)
      end

      it "renders a response with 422 status (i.e. to display the 'new' template)" do
        post booking_requests_url, params: { booking_request: invalid_attributes }
        expect(response).to have_http_status(:unprocessable_entity)
      end
    end
  end

  describe 'PATCH /update' do
    context 'with valid parameters' do
      let(:new_attributes) do
        skip('Add a hash of attributes valid for your model')
      end

      it 'updates the requested booking_request' do
        booking_request = BookingRequest.create! valid_attributes
        patch booking_request_url(booking_request), params: { booking_request: new_attributes }
        booking_request.reload
        skip('Add assertions for updated state')
      end

      it 'redirects to the booking_request' do
        booking_request = BookingRequest.create! valid_attributes
        patch booking_request_url(booking_request), params: { booking_request: new_attributes }
        booking_request.reload
        expect(response).to redirect_to(booking_request_url(booking_request))
      end
    end

    context 'with invalid parameters' do
      it "renders a response with 422 status (i.e. to display the 'edit' template)" do
        booking_request = BookingRequest.create! valid_attributes
        patch booking_request_url(booking_request), params: { booking_request: invalid_attributes }
        expect(response).to have_http_status(:unprocessable_entity)
      end
    end
  end

  describe 'DELETE /destroy' do
    it 'destroys the requested booking_request' do
      booking_request = BookingRequest.create! valid_attributes
      expect do
        delete booking_request_url(booking_request)
      end.to change(BookingRequest, :count).by(-1)
    end

    it 'redirects to the booking_requests list' do
      booking_request = BookingRequest.create! valid_attributes
      delete booking_request_url(booking_request)
      expect(response).to redirect_to(booking_requests_url)
    end
  end
end
